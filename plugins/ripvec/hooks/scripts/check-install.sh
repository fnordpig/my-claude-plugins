#!/usr/bin/env bash
# Ensure ripvec-mcp binary is installed and up-to-date.
#
# Runs as a SessionStart hook BEFORE MCP servers start. Downloads the
# binary here so the MCP command can exec instantly without blocking
...
*)
	echo "ripvec: unsupported platform ${OS}-${ARCH}" >&2
...
if [[ -z "$RIPVEC_VERSION" ]]; then
	echo "ripvec: cannot determine version (no network, no cached binary)" >&2
	exit 0 # Don't block session start
fi

EXPECTED="${RIPVEC_VERSION}:${TARGET}"

# Fast path: binary exists and matches
...
if command -v curl &>/dev/null; then
	...
	echo "ripvec: no curl/wget — install manually: cargo install ripvec-mcp" >&2
	exit 0
fi
...
echo "ripvec-mcp v${RIPVEC_VERSION} installed."
